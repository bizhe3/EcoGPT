...
def main():
    ...
    #  if str(training_args.torch_dtype).lower() in ["bfloat16", "bf16"] else torch.float16
    # Load model
    value_model = AutoModelForSequenceClassification.from_pretrained(
        training_args.reward_model_path, trust_remote_code=model_args.trust_remote_code, num_labels=1, torch_dtype = dtype, low_cpu_mem_usage = True, device_map={"": train_device},
    )
    reward_model = AutoModelForSequenceClassification.from_pretrained(
        training_args.reward_model_path, trust_remote_code=model_args.trust_remote_code, num_labels=1, torch_dtype = dtype, low_cpu_mem_usage = True, device_map={"": rm_device},
    )
    policy = AutoModelForCausalLM.from_pretrained(
        training_args.sft_model_path, trust_remote_code=model_args.trust_remote_code, torch_dtype = dtype, low_cpu_mem_usage = True, device_map={"": train_device},
    )
    ...
    if peft_config is None:
        ref_policy = AutoModelForCausalLM.from_pretrained(
            training_args.sft_model_path, trust_remote_code=model_args.trust_remote_code, torch_dtype = dtype, low_cpu_mem_usage = True, device_map={"": rm_device},
        )
    else:
        ref_policy = None
    ...
    def preprocess_function(examples):
        ...
        system_prompts = examples.get("system_prompt", [""] * len(examples["conversations"]))
        convs_batch = examples["conversations"]  # list[ list[{"from","value"}] ]

        for i, conv in enumerate(convs_batch):
            if not conv or len(conv) == 0:
                continue

            # 只保留合法角色
            conv = [m for m in conv if m.get("from") in roles and m.get("value") is not None]
            if len(conv) == 0:
                continue

            sys = system_prompts[i] if system_prompts else ""

            # 我们把对话跑一遍：每遇到一个 human，就构造一个“到此为止”的 prompt
            history_pairs = []
            pending_human = None
            ...
            for m in conv:
                ...
                if r == "human":
                    # 遇到新 human：先把它作为 pending（等待后续 gpt 补全）
                    pending_human = v

                    # 关键：即使没有 gpt，也要产出一个 prompt 给 PPO 生成
                    msgs = history_pairs + [[pending_human, ""]]  # 给模板一个空答案占位
                    prompt = prompt_template.get_prompt(messages=msgs, system_prompt=sys)

                    tokenized = tokenizer(prompt, padding=False, truncation=True, max_length=max_source_length)
                    if len(tokenized["input_ids"]) > 0:
                        new_examples["input_ids"].append(tokenized["input_ids"])

                elif r == "gpt":
                    # 如果有 pending human，就把它和当前 gpt 组成一对，纳入 history
                    if pending_human is not None:
                        history_pairs.append([pending_human, v])
                        pending_human = None
    ...
    logger.info(f"value  device: {next(value_model.parameters()).device}")
    logger.info(f"reward device: {next(reward_model.parameters()).device}")
